...
light | dark | contrast
    ...
    paycycleStartDay: 25, // 1..31 (si el mes no tiene ese día, 
...
name:"Salud", kind:"expense"},
      ...
      {id:"exp_subs", name:"Suscripciones", 

kind:"expense"},
      {id:"exp_house", name:"Hogar", kind:"expense"},
      {id:"exp_unexp", name:"Imprevistos", 
...
default: none (0)
  ...
  const state = {
    ...
    settings: null,
    route: "panel",
    periodOffset: 0, // 0 = periodo actual, -1 

anterior, +1 siguiente
    range: { enabled:false, startYMD:"", endYMD:"" }, // rango personalizado (inclusive)
    rangeDraft: { 

startYMD:"", endYMD:"" }, // valores del selector antes de aplicar
    cache: {
      tx: [],
      tr: []
    }
...
...structuredClone(DEFAULT_SETTINGS), ...s };
      s.accounts = (s.accounts && s.accounts.length) ? s.accounts : structuredClone

(DEFAULT_SETTINGS.accounts);
      s.categories = (s.categories && s.categories.length) ? s.categories : structuredClone
...
U.el("div",{class:"h1",text:"Error iniciando la app"}),
      U.el("div",{class:"tiny muted", style:"margin-top:8px"}, "No se ha 

podido abrir la base de datos del navegador (IndexedDB). Prueba en otro navegador o revisa permisos.")
    ]));
...
["panel","movimientos","presupuesto","cuentas","ajustes"].includes(h) ? h : "panel";
    state.route = ok;
  }
...
const d = new Date(date.getTime());
    if(s.periodMode === "calendar"){
      const base = new Date(d.getFullYear(), 

d.getMonth()+offset, 1, 0,0,0,0);
      const start = base;
...
U.clamp(parseInt(s.paycycleStartDay||1,10) || 1, 1, 31);
...
startThisMonth = new Date(y, m, startThisMonthDay, 0,0,0,0);
...
startThisMonth;
      ...
      const dimPrev = daysInMonth(prev.getFullYear(), 

prev.getMonth());
      const dayPrev = Math.min(startDay, dimPrev);
      start0 = new Date(prev.getFullYear(), prev.getMonth
...
const tmp = new Date(start0.getFullYear(), start0.getMonth()+offset, 1);
      const dimTmp = daysInMonth(tmp.getFullYear(), 

tmp.getMonth());
      const dayTmp = Math.min(startDay, dimTmp);
      start = new Date(tmp.getFullYear(), tmp.getMonth(), 
...
dimEnd = daysInMonth(tmpEnd.getFullYear(), tmpEnd.getMonth());
    const dayEnd = Math.min(startDay, dimEnd);
    const 

end = new Date(tmpEnd.getFullYear(), tmpEnd.getMonth(), dayEnd, 0,0,0,0);
    return { start, end };
  }
...
{month:"long",year:"numeric"});
    ...
    const a = start.toLocaleDateString("es-ES",{day:"2-

digit",month:"short",year:"numeric"});
    const b = new Date(end.getTime()-1).toLocaleDateString("es-ES",{day:"2-
...

{
    ...
    const da = String(d.getDate()).padStart
...
endInclusiveDate.toLocaleDateString("es-ES",{day:"2-digit",month:"2-digit",year:"numeric"});
    return `${a} – ${b}`;
  }
...
false
  ...
  function ensureRangeDraft(){
    ...
    if(!state.rangeDraft) state.rangeDraft = 

{startYMD:"", endYMD:""};
    if(!state.rangeDraft.startYMD || !state.rangeDraft.endYMD){
      state.rangeDraft.startYMD = 
...
meta.startYMD;
  ...
  const startInp = U.el("input",{class:"input", type:"date", style:"max-width:160px"});
    const endInp = U.el("input",

{class:"input", type:"date", style:"max-width:160px"});
    startInp.value = state.rangeDraft.startYMD;
    endInp.value = 

state.rangeDraft.endYMD;
...
eD){ U.toast("Elige fecha inicio y fecha fin."); return; }
      if(eD.getTime() < sD.getTime()){ U.toast("La fecha fin no puede ser 

anterior a inicio."); return; }
...
("div",{},[U.el("div",{class:"tiny muted",text:"Fin"}), endInp]),
      applyBtn,
      resetBtn,
      badge
    ]);
...
());
    ...
    let guard = 0;
    while(p.start.getTime() < 

meta.end.getTime() && guard < 200){
      guard++;
      const os = Math.max(meta.start.getTime(), p.start.getTime());
      const 

oe = Math.min(meta.end.getTime(), p.end.getTime());
      if(oe > os){
        const frac = (oe - os) / (p.end.getTime() - 
...
----------
 ...
 function catById(id){ return categories().find(c=>c.id===id) || null; }
  function accById(id){ return accounts().find
...
start.getTime();
    ...
    const [tx, tr] = await Promise.all([
      DB.listTxByRange(state.db, startMs, 
...
= await Promise.all([DB.listAllTx(state.db), DB.listAllTr(state.db)]);
    tx.sort((a,b)=>b.dateMs - a.dateMs);
    tr.sort((a,b)
...
const id = t.accountId || "cash";
      ...
      const delta = (t.type==="income") ? Number(t.amount||0) : 
...
map.set(tr.fromAccountId, (map.get(tr.fromAccountId)||0) - amt);
      map.set(tr.toAccountId, (map.get(tr.toAccountId)||0) 
...
"cash";
      ...
      map.set(id, (map.get(id)||0) 

+ delta);
    }
    for(const tr of all.tr){
      const amt = Number(tr.amount||0);
      map.set(tr.fromAccountId, (map.get

(tr.fromAccountId)||0) - amt);
...
function computeBalancesSnapshots(cutoffMsList){
    const { tx, tr } = await loadAllData();
    const cutoffs = Array.from(new 

Set((cutoffMsList||[]).map(x=>Number(x)).filter(x=>Number.isFinite(x)))).sort((a,b)=>a-b);

    const events = [];
    for(const t 

of tx) events.push({dateMs: Number(t.dateMs||0), kind:"tx", t});
    for(const x of tr) events.push({dateMs: Number

(x.dateMs||0), kind:"tr", tr:x});
    events.sort((a,b)=>a.dateMs-b.dateMs);

    const cur = new Map();
    for(const a of accounts

()) cur.set(a.id, Number(a.initialBalance||0));
...
(cur.get(id)||0) + delta);
    ...
    function applyTr(x){
      ...
      cur.set(x.fromAccountId, (cur.get

(x.fromAccountId)||0) - amt);
...
{class:"row", style:"gap:8px"},[
      ...
      U.el("button",{class:"btn small", text:"▶", onclick: ()=>{ state.periodOffset 
...
+= 1; render(); }})
  ...
  function exportBtn(onclick, text="Exportar XLSX"){
    return U.el("button",{class:"btn small", text, 
...
color"]')?.setAttribute("content", t==="dark" ? "#0b1220" : "#f2f4f7");
  }

  function ensureNum(n){ const x = Number(n); 

return Number.isFinite(x) ? x : 0; }

  // ---------- Modal: Add/Edit transaction ----------
  function txFormNode({tx=null, 
...
["expense","income"].forEach(v=>{
      typeSel.appendChild(U.el("option",{value:v, text: v==="expense" ? "Gasto" : 

"Ingreso"}));
    });
    typeSel.value = tx?.type || defaultType;

    const amountInp = U.el("input",{class:"input", 

placeholder:"Importe (ej: 12,50)", inputmode:"decimal"});
    amountInp.value = tx ? U.number2(tx.amount).replace(/\./g,"") : 

"";

    const dateInp = U.el("input",{class:"input", type:"date"});
    dateInp.value = tx ? new Date(tx.dateMs).toISOString

().slice(0,10) : U.todayISO();

    const accSel = U.el("select",{});
    for(const a of accounts()){
      accSel.appendChild(U.el

("option",{value:a.id, text:a.name}));
    }
...
for(const c of list){
      ...
      if(tx?.categoryId && list.some

(c=>c.id===tx.categoryId)) catSel.value = tx.categoryId;
    }
    refreshCats();
...
= U.el("div",{class:"grid", style:"gap:10px"},[
      U.el("div",{},[U.el("div",{class:"tiny muted",text:"Tipo"}), typeSel]),
      U.el

("div",{},[U.el("div",{class:"tiny muted",text:"Importe"}), amountInp]),
      U.el("div",{},[U.el("div",{class:"tiny 

muted",text:"Fecha"}), dateInp]),
      U.el("div",{},[U.el("div",{class:"tiny muted",text:"Cuenta"}), accSel]),
      U.el("div",{},

[U.el("div",{class:"tiny muted",text:"Categoría"}), catSel]),
      U.el("div",{},[U.el("div",{class:"tiny muted",text:"Nota"}), 
...
_forceType || (tx?.type) || "expense";
    const {node, getValue, amountInp} = txFormNode({tx, defaultType});

    const saveBtn = 

U.el("button",{class:"btn primary", text: isEdit ? "Guardar" : "Añadir"});
    const cancelBtn = U.el("button",{class:"btn", 
...
amountInp.focus();
      ...
      modal.close();
      U.toast(isEdit ? "Movimiento 

actualizado." : "Movimiento añadido.");
      await render(); // refresh
    };
  }

  // ---------- Modal: Quick add (Gasto / Ingreso / 
...
("div",{class:"tiny muted", text:"¿Qué quieres añadir?"}),
      U.el("button",{class:"btn primary", text:"Gasto", onclick: ()=>{ 

modal.close(); openTxModal({mode:"add", tx:null, _forceType:"expense"}); }}),
      U.el("button",{class:"btn primary", 

text:"Ingreso", onclick: ()=>{ modal.close(); openTxModal({mode:"add", tx:null, _forceType:"income"}); }}),
      U.el

("button",{class:"btn", text:"Transferencia entre cuentas", onclick: ()=>{ modal.close(); openTransferModal({}); }}),
    ]);
...
----------
  function openTransferModal({tr=null}={}){
    const isEdit = !!tr;
    const dateInp = U.el("input",{class:"input", 
...
("select",{});
    ...
    for(const a of accounts()){
      fromSel.appendChild(U.el("option",{value:a.id, 

text:a.name}));
      toSel.appendChild(U.el("option",{value:a.id, text:a.name}));
    }
    fromSel.value = tr?.fromAccountId || 

"bank";
    toSel.value = tr?.toAccountId || "cash";

    const amountInp = U.el("input",{class:"input", placeholder:"Importe (ej: 

50,00)", inputmode:"decimal"});
...
("textarea",{placeholder:"Nota (opcional)"});
    ...
    const form = U.el("div",{class:"grid", 

style:"gap:10px"},[
      U.el("div",{},[U.el("div",{class:"tiny muted",text:"Fecha"}), dateInp]),
      U.el("div",{},[U.el("div",

{class:"tiny muted",text:"Desde"}), fromSel]),
...
muted",text:"Nota"}), noteInp]),
    ]);

    const saveBtn = U.el("button",{class:"btn primary", text: isEdit ? "Guardar" : 
...
amount = U.parseAmount(amountInp.value);
      if(!amount || amount<=0){ U.toast("Importe no válido."); amountInp.focus(); 
...
categories().filter(x=>x.kind==="expense")){
      const capBase = ensureNum(state.settings.budgets?.[c.id] || 0);
      const cap 

= capBase ? (capBase * capFactor) : 0;
      const spent = ensureNum(exp.get(c.id)||0);
      const remaining = cap ? (cap - spent) : 
...
`ControlGastos_Panel_${label.replace(/\s+/g,"_")}.xlsx`,
      sheets: [
...
(periodo)", rows: rowsByAccountPeriod(txList, trList), currencyCols:[1]},
        {name:"Movimientos", rows: rowsMovements

(txList), currencyCols:[5]},
...
class="kpi"><div class="label">Ingresos</div><div class="value">${escapeHtml(U.money(sums.income, 

state.settings.currency))}</div></div>
        <div class="kpi"><div class="label">Gastos</div><div class="value">

${escapeHtml(U.money(sums.expense, state.settings.currency))}</div></div>
        <div class="kpi"><div 

class="label">Balance</div><div class="value">${escapeHtml(U.money(sums.balance, state.settings.currency))}

</div></div>
      </div>
    `;
    const byCatRows = rowsByCategory(txList).slice(1).map(r=>[r[0], U.money(r[1], 

state.settings.currency)]);
    const byAccRows = rowsByAccountPeriod(txList, trList).slice(1).map(r=>[r[0], U.money(r[1], 
...
rows = categories().filter(c=>c.kind==="expense").map(c=>{
      const cap = ensureNum(state.settings.budgets?.[c.id]||0);
      

const spent = ensureNum(exp.get(c.id)||0);
      const rem = cap ? (cap - spent) : 0;
      return [c.name, U.money(cap, 

state.settings.currency), U.money(spent, state.settings.currency), U.money(rem, state.settings.currency)];
    });
    const html 

= tableHtml(["Categoría","Tope","Gastado","Restante"], rows, new Set([1,2,3]));
    Print.printHtml({title:"Presupuesto 

(Topes)", subtitle:`Periodo: ${label}`, html});
  }
...
computeBalancesSnapshots([startMs, endMs]);
    const balancesStart = snap.snaps.get(startMs) || new Map();
    const 

balancesEnd = snap.snaps.get(endMs) || new Map();
...
rangeControls(),
    ...
    const kpis = U.el("div",{class:"grid cols2"},[
      U.el("div",{class:"kpi"},[U.el("div",{class:"label",text:"Ingresos"}), 

U.el("div",{class:"value",text:U.money(sums.income, state.settings.currency)})]),
      U.el("div",{class:"kpi"},[U.el("div",

{class:"label",text:"Gastos"}), U.el("div",{class:"value",text:U.money(sums.expense, state.settings.currency)})]),
      U.el

("div",{class:"kpi"},[U.el("div",{class:"label",text:"Balance"}), U.el("div",{class:"value",text:U.money(sums.balance, 

state.settings.currency)})]),
      U.el("div",{class:"kpi"},[U.el("div",{class:"label",text:"Tasa de ahorro (aprox.)"}), U.el

("div",{class:"value",text: (sums.income>0 ? ( (sums.balance/sums.income)*100 ).toFixed(1).replace(".",",")+"%" : "—")})]),
...
spent = ensureNum(exp.get(c.id)||0);
      if(spent > cap){
        alerts.push(`${c.name}: superado el tope (${U.money(spent, 

state.settings.currency)} / ${U.money(cap, state.settings.currency)})`);
      }
    }

    const alertsNode = U.el("div",{class:"card", 
...
{class:"badge " + (alerts.length? "danger":"success"), text: alerts.length ? `${alerts.length} aviso(s)` : "Sin avisos"})
      ]),
...
U.el("div",{class:"h2",text:"Gasto por categoría (Top)"}),
        U.el("a",{href:"#/presupuesto", class:"btn small ghost", 
...
("tbody",{}, catItems.map(it=>{
          return U.el("tr",{},[
            U.el("td",{text: catById(it.catId)?.name || it.catId}),
            U.el

("td",{class:"num", text: U.money(it.amt, state.settings.currency)})
          ]);
        }))
      ]) : U.el("div",{class:"tiny 

muted",text:"Aún no hay gastos en el periodo."})
    ]);

    // By account (period net)
    const net = groupByAccountNet(txList, 

trList);
    const byAcc = U.el("div",{class:"card", style:"margin-top:12px"},[
      U.el("div",{class:"h2",text:"Variación por 
...
{class:"num", text: U.money(v, state.settings.currency)})
          ]);
        }))
      ])
    ]);

    return U.el("div",{},[head, kpis, 

alertsNode, byCat, byAcc]);
  }
...
U.el("select",{});
    [["all","Todos"],["income","Ingresos"],["expense","Gastos"]].forEach(([v,t])=> typeSel.appendChild

(U.el("option",{value:v,text:t})));

    const accSel = U.el("select",{});
    accSel.appendChild(U.el("option",{value:"all", 
...
categories().filter(c=>c.kind===kind) : categories();
      for(const c of list){
        catSel.appendChild(U.el("option",{value:c.id, 

text:`${c.name} (${c.kind==="income"?"Ingreso":"Gasto"})`}));
      }
    }
    refreshCatOptions();

    const transferBtn = U.el

("button",{class:"btn small", text:"Transferir", onclick: ()=> openTransferModal({})});

    const addBtn = U.el("button",

{class:"btn primary", text:"Añadir", onclick: ()=> openQuickAddModal()});
...
`ControlGastos_Movimientos_${label.replace(/\s+/g,"_")}.xlsx`,
    ...
    const head = sectionHeader("Movimientos", U.el("div",{class:"row", style:"gap:8px"},[
      rangeControls(), printB, 

exportB, transferBtn, addBtn
    ]));
...
(t.categoryId||"")!==filters.category) return false;
...
items.length){
        listNode.appendChild(U.el("div",{class:"card"}, U.el("div",{class:"tiny muted",text:"No hay movimientos 
...
acc = accById(t.accountId)?.name || "";
        const date = new Date(t.dateMs).toLocaleDateString("es-ES",{day:"2-
...
U.confirmDialog({title:"Borrar movimiento", message:"Se borrará este movimiento. ¿Quieres continuar?", okText:"Borrar", 
...
{class:"title",text: `${cat || (t.type==="income"?"Ingreso":"Gasto")}`}),
            U.el("div",{class:"meta",text: `${date} · 

${acc}${t.note? " · "+t.note : ""}`})
          ]),
          U.el("div",{class:"right"},[
            U.el("div",{class:`amount ${amtCls}`, text: `

${amtSign}${U.money(t.amount, state.settings.currency)}`}),
            U.el("div",{class:"row", style:"gap:8px"},[editBtn, 
...
()=>{ filters.category = catSel.value; renderList(); });

    const filterCard = U.el("div",{class:"card", style:"margin:10px 0 
...
{},[U.el("div",{class:"tiny muted",text:"Buscar"}), qInp]),
        U.el("div",{},[U.el("div",{class:"tiny muted",text:"Tipo"}), 

typeSel]),
        U.el("div",{},[U.el("div",{class:"tiny muted",text:"Cuenta"}), accSel]),
        U.el("div",{},[U.el("div",{class:"tiny 

muted",text:"Categoría"}), catSel]),
      ])
    ]);
...
function viewPresupuesto(){
    ...
    // Presupuesto (Topes) es un ajuste: se define por 

categoría y no depende de un rango personalizado.
    // Para el "gastado" usamos siempre el periodo actual configurado (mes 

natural o cobro-a-cobro).
    const range = currentPeriodRange();
    const label = periodLabel(range);

    // Cargar datos del 

periodo (ignorando rango personalizado)
    const startMs = range.start.getTime();
    const endMs = range.end.getTime();
    

state.cache.tx = await DB.listTxByRange(state.db, startMs, endMs);
    state.cache.tr = await DB.listTrByRange(state.db, 

startMs, endMs);
    state.cache.loadedRange = { startMs, endMs };

    const head = sectionHeader("Presupuesto (Topes)", U.el
...
("th",{text:"Categoría"}),
      ...
      U.el("th",{text:"Gastado", style:"text-
...
(state.settings.budgets?.[cat.id] || 0);
...
: 0;
      ...
      const capInp = U.el("input",{class:"input", inputmode:"decimal", 

style:"text-align:right; max-width:140px"});
...
state.settings.budgets[cat.id];
          else state.settings.budgets[cat.id] = ensureNum(v);
          await DB.saveSettings(state.db, 
...
state.settings);
      ...
      U.el("td",{},[
          ...
          U.el("div",{class:"progress", style:"margin-

top:6px"}, U.el("div",{style:`width:${(ratio*100).toFixed(1)}%`}))
        ]),
        U.el("td",{class:"num"}, capInp),
        U.el("td",

{class:"num", text: U.money(spent, state.settings.currency)}),
        U.el("td",{class:"num", text: cap>0 ? U.money(remaining, 
...
{class:"tiny muted", style:"margin-top:10px"},
...
("input",{class:"input", placeholder:"Nombre de nueva categoría"});
    const addKind = U.el("select",{});
    

addKind.appendChild(U.el("option",{value:"expense", text:"Gasto"}));
    addKind.appendChild(U.el("option",
...
addBtn.onclick = async ()=>{
      ...
      if(!name){ U.toast("Escribe un nombre."); 

addName.focus(); return; }
...
("Categoría añadida.");
    ...
    catCard.appendChild(U.el

("div",{class:"grid cols2"},[
      U.el("div",{},[U.el("div",{class:"tiny muted",text:"Nombre"}), addName]),
      U.el("div",{},

[U.el("div",{class:"tiny muted",text:"Tipo"}), addKind]),
    ]));
    catCard.appendChild(U.el("div",{class:"row", style:"justify-

content:flex-end;margin-top:10px"}, addBtn));
...
{class:"tiny muted", text:"El saldo actual se calcula: saldo inicial + histórico de movimientos/transferencias."})
    ]);
...
a.initialBalance ? U.number2(a.initialBalance).replace(/\./g,"") : "";
      inp.addEventListener("change", async ()=>{
        

a.initialBalance = ensureNum(U.parseAmount(inp.value));
        await DB.saveSettings(state.db, s);
        U.toast("Saldo inicial 
...
("div",{class:"h2", text:"Copia de seguridad"}),
      U.el("hr",{class:"sep"}),
      U.el("div",{class:"tiny muted", text:"Exporta 

un JSON para guardar todos tus datos. Puedes restaurarlo más tarde en el mismo navegador/dispositivo."})
    ]);

    const 

expBtn = U.el("button",{class:"btn primary", text:"Exportar JSON"});
    expBtn.onclick = async ()=>{
      const payload = await 

DB.exportAll(state.db);
...
setTimeout(()=>{ URL.revokeObjectURL(a.href); a.remove(); }, 350);
    ...
    const copyBtn = U.el("button",{class:"btn", 
...
(state.db);
        ...
        if(navigator.clipboard && typeof 

navigator.clipboard.writeText==="function"){
          await navigator.clipboard.writeText(text);
          U.toast("JSON copiado al 
...
U.el("input",{class:"input", type:"file", accept:"application/json,.json"});
    const impBtn = U.el("button",{class:"btn danger", 

text:"Importar JSON (restaurar)"});
    impBtn.onclick = async ()=>{
      const f = impInp.files?.[0];
      if(!f){ U.toast("Selecciona 

un archivo JSON."); return; }
      const ok = await U.confirmDialog({
        title:"Restaurar copia",
        message:"Se reemplazarán 

los datos actuales por los del backup. ¿Continuar?",
        okText:"Restaurar",
        cancelText:"Cancelar"
      });
      if(!ok) return;
...
try{
        ...
        if(!payload || !payload.settings || !Array.isArray

(payload.tx) || !Array.isArray(payload.tr)){
          U.toast("El archivo no parece un backup válido.");
          return;
        }
        await 

DB.importAll(state.db, payload);
...
completo del backup JSON…",
        style:"min-height:220px;font-family:ui-monospace, SFMono-Regular, Menlo, Monaco, 

Consolas, 'Liberation Mono', 'Courier New', monospace;white-space:pre;"
      });
      const tip = U.el("div",{class:"tiny muted", 

html:"Consejo: abre el .json en el ordenador, copia todo (Ctrl+A, Ctrl+C) y envíatelo por WhatsApp. En el móvil, copia el texto 

del chat y pégalo aquí."});

      const doBtn = U.el("button",{class:"btn danger", text:"Restaurar"});
      const cancelBtn = U.el

("button",{class:"btn", text:"Cancelar"});

      const modal = U.openModal({
        title:"Pegar JSON",
        contentNode: U.el

("div",{class:"grid", style:"gap:10px"},[tip, ta]),
...
modal.close();
      doBtn.onclick = async ()=>{
        ...
        if(!raw){ U.toast("Pega el 

contenido JSON."); ta.focus(); return; }

        const ok = await U.confirmDialog({
          title:"Restaurar copia",
          message:"Se 

reemplazarán los datos actuales por los del backup. ¿Continuar?",
          okText:"Restaurar",
          cancelText:"Cancelar"
        });
...
backupCard.appendChild(U.el("div",{class:"grid cols2"},[
      U.el("div",{},[U.el("div",{class:"row", style:"gap:10px;flex-

wrap:wrap;"},[expBtn, copyBtn])]),
      U.el("div",{},[
        impInp,
...
const wipeBtn = U.el("button",{class:"btn danger", 

text:"Borrar todos los datos"});
    wipeBtn.onclick = async ()=>{
      const ok = await U.confirmDialog({title:"Borrar todo", 

message:"Esto borrará todos los movimientos, transferencias y ajustes. Acción irreversible. ¿Continuar?", okText:"Borrar", 

cancelText:"Cancelar"});
      if(!ok) return;
      await DB.importAll(state.db, {settings: structuredClone(DEFAULT_SETTINGS), 
...
("div",{class:"h2", text:"Mantenimiento"}),
      U.el("hr",{class:"sep"}),
      U.el("div",{class:"row space"},[
        U.el("div",

{class:"tiny muted", text:"Versión: v1 · Datos en IndexedDB"}),
        wipeBtn
      ])
    ]);

    return U.el("div",{},[head, periodCard, 

themeCard, catCard, accCard, backupCard, footer]);
  }
...
setActiveNav();
    ...
    try{
      let node;
      if(state.route==="panel") node = 

await viewPanel();
      else if(state.route==="movimientos") node = await viewMovimientos();
      else if

(state.route==="presupuesto") node = await viewPresupuesto();
      else if(state.route==="cuentas") node = await 
...
style:"margin-top:8px"}, "Ha ocurrido un error renderizando la pantalla. Revisa la consola.")
      ]));
    }finally{
